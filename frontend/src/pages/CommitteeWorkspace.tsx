...
import { CalendarPlus, Crown, Eye, FileText, ListPlus, QrCode, ShieldCheck, Trash2, Upload, UserPlus, Video } from "lucide-react";
      ...
      {tab === "Overview" && (
        <div className="space-y-4">
          {(() => {
            ...
            const withWindow = detail.meetings.map((m) => {
            ...
            const ongoing = withWindow.filter(({ start, end }) => start <= nowMs && nowMs <= end);
            const upcoming = withWindow
              .filter(({ start }) => start > nowMs && start - nowMs < 24 * 60 * 60 * 1000)
              .sort((a, b) => a.start - b.start);

            if (ongoing.length === 0 && upcoming.length === 0) {
              return (
                <div className="card border-dashed border-slate-200 bg-slate-50/50 dark:border-slate-700 dark:bg-slate-900/40">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">No meeting in session</p>
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    When a meeting is underway (between start and end time), it appears here with a live badge and attendance QR for the Chairperson or Secretary.
                  </p>
                </div>
              );
            }
            ...
            const renderCard = (
              m: (typeof detail.meetings)[number],
              mode: "live" | "upcoming",
            ) => (
              <div
                key={`${mode}-${m.id}`}
                className={
                  mode === "live"
                    ? "card border-brand-300 bg-gradient-to-r from-brand-50 to-white dark:border-brand-800 dark:from-brand-950/40 dark:to-slate-800"
                  ...
                  <div>
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      <span
                        className={
                          mode === "live"
                            ? "inline-flex items-center rounded-full bg-emerald-100 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"
                            : "inline-flex items-center rounded-full bg-amber-100 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-amber-800 dark:bg-amber-950 dark:text-amber-300"
                        }
                      >
                        {mode === "live" ? "Live now" : "Starting soon"}
                      </span>
                      <span className="text-xs text-slate-500 dark:text-slate-400">{m.reference}</span>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {mode === "live" && canMembers && (
                      <button
                        ...
                        onClick={() => void openAttendanceProjector(m)}
                      >
            ...
            return (
              <div className="space-y-3">
                {ongoing.map(({ m }) => renderCard(m, "live"))}
                {upcoming.map(({ m }) => renderCard(m, "upcoming"))}
              </div>
            );
          })()}
          ...
          <div className="mb-4 flex items-center gap-2">
            {["All", "Open", "In Progress", "Pending Verification", "Overdue", "Completed"].map((s) => (
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-xs font-medium ${statusFilter === s ? "bg-brand-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200 dark:hover:bg-slate-600"
                  }`}
              >
                {s}
              </button>
            ...
            <tbody>
              {actions.map((a) => (
                <tr
                  ...
                  className="cursor-pointer border-b border-slate-100 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700/60"
                >
